...
async def settings_query(bot, query):
    ...
    if type == "main":
        ...
        if _bot is not None:
            buttons.append([InlineKeyboardButton(_bot['name'], callback_data="settings")])
    else:
        buttons = []
    ...
    await query.message.edit_text(
        ...
        bot = await CLIENT.add_bot(bot, query)  # Use 'await' before 'CLIENT'
        if bot != True:
            return
        ...
        await query.message.delete()
        user = await CLIENT.add_session(bot, query)  # Use 'await' before 'CLIENT'
        if user != True:
            return
        await query.message.reply_text(
        ...
        await query.message.delete()
        chat_ids = await Client.ask(chat_id=query.message.chat.id,
      ...
      await query.message.edit_text(
         "<b><u>CUSTOM CAPTION</b></u>\n\n<b>You can set a custom caption to videos and documents. Normally use its default caption</b>\n\n<b><u>AVAILABLE FILLINGS:</b></u>\n- <code>{filename}</code> : Filename\n- <code>{size}</code> : File size\n- <code>{caption}</code> : default caption",
         reply_markup=InlineKeyboardMarkup(buttons))
     ...
     buttons.append([InlineKeyboardButton('back', 
                      callback_data="settings#main")])
     await query.message.edit_text(
        "<b><u>DATABASE</u>\n\nDatabase is required for store your duplicate messages permanent. Otherwise, stored duplicate media may disappear when the bot restarts.</b>",
        reply_markup=InlineKeyboardMarkup(buttons))
    ...
    btn.append([InlineKeyboardButton('back', 'settings#main')])
    await query.message.edit_text(
        text='<b><u>KEYWORDS</u></b>\n\n**Files with these keywords in the file name will forward**',
        reply_markup=InlineKeyboardMarkup(btn))
...
async def filters_buttons(user_id):
    ...
    buttons = [
        [
            InlineKeyboardButton('🏷️ Forward tag', callback_data=f'settings_#updatefilter-forward_tag-{filter["forward_tag"]}'),
            InlineKeyboardButton('✅' if filter['forward_tag'] else '❌', callback_data=f'settings#updatefilter-forward_tag-{filter["forward_tag"]}')
        ],
        [
            InlineKeyboardButton('🖍️ Texts', callback_data=f'settings_#updatefilter-text-{filters["text"]}'),
        ...
        [
            InlineKeyboardButton('▶️ Skip duplicate', callback_data=f'settings_#updatefilter-duplicate-{filter["duplicate"]}'),
...
async def next_filters_buttons(user_id):
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton('🔒 Secure message', callback_data=f'settings_#updatefilter-protect-{filter["protect"]}'),
        ...
        [
            InlineKeyboardButton('♦️ keywords ♦️', callback_data='settings#get_keyword')
        ],
        [
            InlineKeyboardButton('⫷ back', callback_data="settings#filters")
        ]
    ]
    return InlineKeyboardMarkup(buttons) v
